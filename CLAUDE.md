...
- **`<title>` is not the visible heading.** Section pages use `UI[locale].<section>.metaTitle` (a descriptive phrase) for `<title>`/`og:title`, and `title` (lower-case) for the h1. The home title is `SITE[locale].homeTitle`.
